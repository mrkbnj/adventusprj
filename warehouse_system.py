...
def initialize_file():
    ...
    default_dfs = {
        ...
        "pullouts_w2": pd.DataFrame(columns=["Set ID", "Equipment Type", "Brand/Model", "Serial Number", "Checked By", "Shelf", "Status", "Remarks", "Pull Reason", "Date"]),
    }
...
def update_item():
    ...
    df_items.at[index, "Brand/Model"]    = new_brand
    df_items.at[index, "Serial Number"]  = new_serial
    df_items.at[index, "Checked By"]     = new_checked_by
    df_items.at[index, "Shelf"]          = new_shelf
    df_items.at[index, "Status"]         = new_status
    df_items.at[index, "Remarks"]        = new_remarks
    save_warehouse_1(df_items, df_shelves)
...
def pull_search_live(event=None):
    """Filter the warehouse table live as the user types in the pull search box.
    Only affects the warehouse view (for item selection); pull history is filtered separately."""
    keyword = pull_item_entry.get().strip().lower()
    # Only update warehouse tree — never cross into pull history tree
    show_warehouse()
    if keyword:
        df = load_items()
        df = df[df["Hostname"].astype(str).str.lower().str.contains(keyword, na=False)]
...
def _open_qr_gallery(warehouse):
    ...
    tk.Button(toolbar, text="↻ Refresh", command=lambda: _load_gallery(search_var.get()),
...
def search_item():
    ...
    if keyword:
        search_cols = ["Hostname", "Serial Number", "Checked By", "Shelf", "Status", "Remarks"]
        mask = False
...
def filter_pull_history():
    ...
    df = load_pullouts()
    if reason:
        df = df[df["Pull Reason"].str.lower().str.contains(reason, na=False)]
    df = _filter_by_date(df, date_from, date_to)
...
def select_item(event):
    ...
    if selected:
        ...
        _fill_input_fields(values[1], values[2], values[3], values[4], values[5], values[6], values[7])
        pull_item_entry.delete(0, tk.END)
...
def w2_show_warehouse():
    ...
    if keyword:
        mask = (
            ...
            df["Hostname"].str.lower().str.contains(keyword, na=False) |
            df["Serial Number"].astype(str).str.lower().str.contains(keyword, na=False) |
            df["Remarks"].astype(str).str.lower().str.contains(keyword, na=False)
        )
        df = df[mask]
...
def w2_show_pullouts():
    ...
    for _, row in df_po2.iterrows():
        tree_w2_pullouts.insert("", "end", values=tuple(
            row.get(c, "") for c in ["Set ID", "Equipment Type", "Brand/Model", "Shelf", "Status", "Remarks", "Pull Reason", "Date"]))
    try:
...
def w2_pull_search_live(event=None):
    """Filter the W2 warehouse table live as the user types in the pull search box.
    Only affects the warehouse view (for item selection); pull history is filtered separately."""
    keyword = w2_pull_item_entry.get().strip().lower()
    # Always show warehouse tree for item selection
    w2_show_warehouse()
    if keyword:
        df = load_items_w2()
        mask = (
            df["Hostname"].astype(str).str.lower().str.contains(keyword, na=False) |
            df["Set ID"].astype(str).str.lower().str.contains(keyword, na=False) |
            df["Equipment Type"].astype(str).str.lower().str.contains(keyword, na=False)
        )
        df = df[mask]
        _populate_w2_warehouse_tree(df)
...
def w2_undo_pull(event):
    ...
    if not values:
        return
    set_id, eq_type, brand, shelf, status, remarks = values[0], values[1], values[2], values[3], values[4], values[5]
...
def open_label_manager():
    ...
    tree_labels = ttk.Treeview(table_frame_m, columns=("Warehouse", "File", "Date", "Size"), show="headings", height=14, selectmode="extended")
    ...
    def open_selected():
        ...
        # Open only the first selected file
    ...
    def clear_selected():
        selected = tree_labels.selection()
        if not selected:
            messagebox.showwarning("Warning", "Select one or more files to clear", parent=manager); return
        count = len(selected)
        if count == 1:
            filename = tree_labels.item(selected[0], "values")[1]
            prompt = f"Delete '{filename}'?"
        else:
            prompt = f"Delete {count} selected files? This cannot be undone."
        if not messagebox.askyesno("Confirm Clear", prompt, parent=manager):
            return
        failed = []
        for iid in selected:
            full_path = row_paths.get(iid)
            try:
                if full_path and os.path.exists(full_path):
                    os.remove(full_path)
            except Exception as e:
                failed.append(f"{tree_labels.item(iid, 'values')[1]}: {e}")
        load_label_files()
        if failed:
...
tree_w2_pullouts = ttk.Treeview(w2_table_frame,
    columns=("C1","C2","C3","C4","C5","C6","C7","C8"), show='headings')
for col, text, width in zip(
    ("C1","C2","C3","C4","C5","C6","C7","C8"),
    ("Set ID","Equipment Type","Brand/Model","Shelf","Status","Remarks","Pull Reason","Date"),
    (85,115,130,110,80,160,180,130)):
...
# ── Init ──────────────────────────────────────────────────
# Backup the data file before each session (best-effort, non-fatal)
import shutil as _shutil
if os.path.exists(FILE):
    try:
        _shutil.copy2(FILE, FILE + ".bak")
    except Exception:
        pass
